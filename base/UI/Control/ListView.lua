...
function prototype:createItems(info)
    ...
    self.autoContentSize = info.autoContentSize or false
...
function prototype:createItemModel(ccsName)
    ...
    if self.autoContentSize then
        local listSize = self.rootNode:getContentSize()
        local itemSize = node:getContentSize()
        node:setContentSize(cc.size(listSize.width, itemSize.height))
...
function prototype:getItemIndex(data, node)
	for idx, v in ipairs(self.items) do
		if (node and v.node == node) 
            or (data and self.dataCheckFunc(data, v.data)) then
...
function prototype:getSubItemByIdx(idx)
    return self.items[idx].node
end