...
--liuw
function GroupDecoder(field_number, is_repeated, is_packed, key, new_default)
    assert(false, "decoder error :: GroupDecoder")
end
...
function _SkipGroup(buffer, pos, pend)
    error('Tag had invalid wire type. _SkipGroup')
end

function _EndGroup(buffer, pos, pend)
    error('Tag had invalid wire type. _EndGroup')
end
