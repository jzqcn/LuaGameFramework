...
function prototype:refresh(cell_data, idx, sec_idx)
	-- log(cell_data)
	if idx == 1 then
		self.imgRankIcon:loadTexture("resource/Dantiao/csbimages/HeadFrame/icon0.png")
		self.imgRankIcon:setVisible(true)
		self.fntRank:setVisible(false)
	else
		if idx <= 4 then
			self.imgRankIcon:loadTexture(string.format("resource/Dantiao/csbimages/HeadFrame/icon%d.png", idx-1))
			self.imgRankIcon:setVisible(true)
			self.fntRank:setVisible(false)
		else
	...
	self.txtName:setString(cell_data.playerName)
