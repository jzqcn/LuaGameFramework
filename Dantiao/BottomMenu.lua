...
local BET_RANGE = {1, 10, 50, 100, 500}
...
function prototype:enter()
	...
	--设置头像
	sdk.account:getHeadImage(accountInfo.userId, accountInfo.nickName, self.imgIcon, accountInfo.headImage)
	...
	local roomInfo = Model:get("Games/Dantiao"):getRoomInfo()
...
function prototype:setBtnBetBg(betRangeInfo)
	...
	for k,v in ipairs(betRangeInfo) do
		...
		self[name]:loadTextureNormal(string.format("resource/Dantiao/csbimages/Bet/b%d.png",v))
		self[name]:loadTexturePressed(string.format("resource/Dantiao/csbimages/Bet/a%d.png",v))
		self[name]:loadTextureDisabled(string.format("resource/Dantiao/csbimages/Bet/c%d.png",v))
	end
end
...
function prototype:setEnabledContinueBet(isEnabled, lastBetRange)
	...
	if not isEnabled or not lastBetRange or self.userInfo.gold < (BET_MIN_LIMIT*100) then
		...
		for i, v in ipairs(lastBetRange) do
			lastBetValue = lastBetValue + v
		end

		if lastBetValue > 0  then
			-- local accountInfo = Model:get("Account"):getUserInfo()
			local playerInfo = self.userInfo --Model:get("Games/Dantiao"):getUserInfo()
...
function prototype:setSelectBetIndex(index)
	...
	if not self.spriteEff then
	 ...
	 --    for i = 1, 8 do				
	 --        animation:addSpriteFrameWithFile(string.format("resource/Dantiao/csbimages/betBtnEff/%d.png", i))
	 --    end
	 ...
	 --    self.spriteEff = sprite
		...
		local skeletonNode = sp.SkeletonAnimation:create("resource/Dantiao/csbimages/anim/BetEff/Tsitexiao.json", "resource/Dantiao/csbimages/anim/BetEff/Tsitexiao.atlas")
	...
	--[[self[name]:loadTextureNormal(string.format("resource/Dantiao/csbimages/Bet/a%d.png", BET_RANGE[index]), ccui.TextureResType.plistType)]]
	self[name]:loadTextureNormal(string.format("resource/Dantiao/csbimages/Bet/a%d.png", BET_RANGE[index]))
	if self.selIndex then
		local res = string.format("resource/Dantiao/csbimages/Bet/b%d.png", BET_RANGE[self.selIndex])
		self["btnBet_"..self.selIndex]:loadTextureNormal(res)
	end
...
function prototype:onBtnBetLastTouch(sender, eventType)
	if eventType == ccui.TouchEventType.ended then
		self:fireUIEvent("Dantiao.ContinueBet")
	end
end
...
function prototype:onBtnOnlinePlayerTouch(sender, eventType)
	if eventType == ccui.TouchEventType.ended then
		self:fireUIEvent("Dantiao.PlayerList")
	end
end
