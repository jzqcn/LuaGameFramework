...
require "Protol.ShiSanShuiGoldConfig_pb"
...
function class:initialize()
    ...
    net.msg:on(MsgDef_pb.MSG_SHISANSHUI_GOLD_CONFIG, self:createEvent("onGoldConfigData"))
end

function class:requestGetLevelConfig(typeId)
	local request = ShiSanShuiGoldConfig_pb.ShiSanShuiGoldConfigRequest()
	request.typeId = typeId
	net.msg:send(MsgDef_pb.MSG_SHISANSHUI_GOLD_CONFIG, request:SerializeToString())
end
...
function class:onGoldConfigData(data)
	...
	local response = ShiSanShuiGoldConfig_pb.ShiSanShuiGoldConfigResponse()
	response:ParseFromString(data)
	-- log("receive ShiSanShui gold config data")
