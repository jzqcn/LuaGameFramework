...
function prototype:setPlayerInfo(info)
	if not info then
		...
		-- self.imgHead:loadTexture("resource/Longhudou/csbimages/none.png")
	...
	--设置头像
	-- sdk.account:getHeadImage(info.playerId, info.playerName, self.imgHead, info.headimage)
...
function prototype:setPlayerType(isDiviner, ranking)
	if isDiviner then
		self.imgSign:loadTexture("resource/Longhudou/csbimages/HeadFrame/diviner.png")
	else
		ranking = ranking or 1		
		self.imgSign:loadTexture(string.format("resource/Longhudou/csbimages/HeadFrame/rich_No_%d.png", ranking))
	end
end
