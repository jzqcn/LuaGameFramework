...
function prototype:enter()
	...
	self:bindModelEvent("Games/Mushiwang.EVT.PUSH_CLUB_CREATE_ROOM", "onPushClubCreateRoom")
...
function prototype:getConfigInfo(lastParam)
	...
	if lastParam then
		--获取上次记录数据
		local varKey = "Room_Mushiwang_"..self.data.config.currencyType.."_"..self.data.typeValue
	...
	local configData = {
		...
		{key = "C_maxPlayerNums", selType = "single", txtType = "ItemShortText2", default = configInfo["C_maxPlayerNums"] or 1},
		{key = "C_evilNum", selType = "single", txtType = "ItemShortText2", default = configInfo["C_evilNum"] or 1}
...
function prototype:createCardRoomByConfig(typeValue, clubId)
	...
	if configData then
		...
		-----保存配置信息------
		local varKey = "Room_Mushiwang_"..self.data.config.currencyType.."_"..typeValue
		local varStr = json.encode(saveConfigStr)
		db.var:setUsrVar(varKey, varStr)
		log(varStr)
		Model:get("Games/Mushiwang"):requestCreateCardRoom(configInfo, clubId)
		self:fireUIEvent("CreateRoomGame")
	else
		log4warn("[MushiwangConfig::createCardRoomByConfig] get table config data error !!!")
	end
end
