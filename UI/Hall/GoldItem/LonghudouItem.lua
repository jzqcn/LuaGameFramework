...
function prototype:enter()
	...
	self:bindModelEvent("Games/LonghudouGoldConfig.EVT.PUSH_LEVEL_CONFIG_DATA", "onPushLevelConfigData")
	...
	factory:loadDragonBonesData("resource/csbimages/Hall/GoldItem/longhudou_ske.dbbin", "longhudou")
    factory:loadTextureAtlasData("resource/csbimages/Hall/GoldItem/longhudou_tex.json", "longhudou")
    local itemAnimation = factory:buildArmatureDisplay("armatureName", "longhudou")
...
function prototype:exit()
	...
	if self.itemAnimation then
		...
		dragonBones.CCFactory:getFactory():removeDragonBonesData("longhudou")
		dragonBones.CCFactory:getFactory():removeTextureAtlasData("longhudou")
	end
...
function prototype:versionPass()
	Model:get("Games/LonghudouGoldConfig"):requestGetLevelConfig(self.itemInfo.typeId)
end
...
function prototype:onPushLevelConfigData(goldData)
	if not goldData or #goldData == 0 then
		log4ui:warn("[LonghudouItem::onPushLevelConfigData] get level data failed !")
		return
	end
	...
	Model:get(modelName):requestEnterRoom(110001, 110)
end
