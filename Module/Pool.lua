...
function class:getCreator()
	return function(ccb, parent)
		    	return self:getFromPool(function()
		    	    return Tw.Controller:load(ccb, parent)
		    	end, ccb)
		    end
end
...
-------------------------------------------
-- support: ccb file with the "Tw.Controller.load" function
...
function ccbRefactor:startStateForPool(ccb)
	Events.Tracer.initialize(ccb)
	Tw.Controller.ControlManager:push(ccb)
end

function ccbRefactor:cleanStateForPool(ccb)
	Tw.Controller.ControlManager:remove(ccb)
	Events.Tracer.dispose(ccb)
end
